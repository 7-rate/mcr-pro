...
////////////////////////////////////////////////////////////////////////////////
SimpleSerialShell::SimpleSerialShell() : shellConnection( NULL ), m_lastErrNo( EXIT_SUCCESS ), tokenizer( strtok_r ) {
    resetBuffer();

    // simple help.
    addCommand( F( "help" ), SimpleSerialShell::printHelp );
    addCommand( F( "history" ), SimpleSerialShell::printHistory );
    ...
    do {
        ...
        case '\x1b': // Escape sequence
        {
            ...
            if ( c2 == '[' && c3 == 'A' ) {
                // ESC [ A is up arrow
                line_clear();
                char* buf = command_history_up();
                strncpy( linebuffer, buf, SIMPLE_SERIAL_SHELL_BUFSIZE );
                inptr = strlen( linebuffer );
                print( buf );
            } else if ( c2 == '[' && c3 == 'B' ) {
                // ESC [ B is down arrow
                line_clear();
                char* buf = command_history_dw();
                strncpy( linebuffer, buf, SIMPLE_SERIAL_SHELL_BUFSIZE );
                inptr = strlen( linebuffer );
                print( buf );
            } else {
...
int SimpleSerialShell::execute( void ) {
    ...
    command_history_add( linebuffer );
...
int SimpleSerialShell::printHistory( int /*argc*/, char** /*argv*/ ) {
    shell.println( F( "Commands History:" ) );
